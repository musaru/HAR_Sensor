...
import os
os.chdir('F:/PostDoctoral_Research/HAR/Sensor/Dataset/Pamap2/')
# Load data
...
def segment(data, window_size): # data is numpy array
    ...
    return {'data' : np.asarray(X), 'labels': np.asarray(y,dtype=int)}
...
if __name__ == "__main__":
    ...
    npz_dir='F:/PostDoctoral_Research/HAR/Sensor/Dataset/Pamap2/'
    output_path = npz_dir + '/pamap2.npz'
    with open(output_path, 'wb') as f:
        np.savez_compressed(f, data=segment_data['data'], labels=segment_data['labels'])
        #np.savez_compressed(f,segment_data, allow_pickle=True)
...
unique_labels, counts = np.unique(y_loaded, return_counts=True)
