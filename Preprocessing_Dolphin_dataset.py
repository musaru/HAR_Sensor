...
os.chdir('F:/PostDoctoral_Research/HAR/Sensor/Dataset/Daphnet/updated/Daphnet-Freezing-of-Gait-main/dataset_fog_release')
...
def read_files():
    ...
    for i, file in enumerate(files):
        ...
        dataCollection.append(procData)  # Append the DataFrame to the list
        #break; # for testing short version, need to delete later       
    #dataCollection.reset_index(drop=True, inplace=True)
       
        #break; # for testing short version, need to delete later       
    #dataCollection.reset_index(drop=True, inplace=True)
    dataCollection = pd.concat(dataCollection, ignore_index=True)
...
if __name__ == "__main__":
    ...
    X,y = segment(numpy_data, window_size) 
   
    
    npz_dir='F:/PostDoctoral_Research/HAR/Sensor/Dataset/Daphnet/'
    output_path = npz_dir + '/Daphnet.npz'
    with open(output_path, 'wb') as f:
        np.savez_compressed(f, data=np.asarray(X), labels=np.asarray(y))
